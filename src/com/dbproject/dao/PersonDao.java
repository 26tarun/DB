...
public class PersonDao {
	...
	public static int updatePassword(String password, String sjsuid){
		...
		try {  
			conn=ConnectionPool.getConnectionFromPool();
			PreparedStatement pst2 =conn.prepareStatement("SELECT COUNT(*) FROM PERSON WHERE USERID=?  AND PASSWORD IS NULL");
			...
			if(count==1){
				pst = conn  
						.prepareStatement("UPDATE PERSON SET PASSWORD=SHA1(?) "
								+ " WHERE USERID=?");  
